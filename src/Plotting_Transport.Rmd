...
depth_dist_seas <- seas_nemo_results %>%
  ...
  scale_y_continuous(name = "Mean surface ocean to burial site distance (km)") +
  ggtitle("Water depth and mean travel distance in Mediterranean subbasins")
...
uk37_dist_plot <- uk37_results_datatable %>%
  ...
  scale_color_brewer(palette = "Greys", direction = -1) +
  labs(color = "Sinking speed (m/d)") +
  scale_x_continuous(name = "Mean surface ocean to burial site distance (km)") +
  scale_y_continuous(name = expression(paste("Nov to May ", SST[UK37] - SST[site], " (\u00b0C)"))) +
  ggtitle(expression("Modeled Nov-May transport distance and proxy offset for " * U[37]^{K} *" sites"))

uk37_dist_plot

ggsave("seasonal transport distance vs uk37 seasonal offset.pdf", uk37_dist_plot)
```
...
ggsave("annual transport distance vs uk37 seasonal offset.pdf", uk37_dist_plot)
```
...
ggsave("travel distance vs tex86 offset.pdf",tex86_dist_plot)
```
...
ggsave("modeled vs tex86 offset - only deep sites.pdf",tex86_offset_plot_deep)
```
...
ggsave("travel distance vs tex86 offset - only deep sites.pdf",tex86_dist_plot_deep)
```
...
tex86_offset_sp6_plot <- tex86_results_datatable %>%
  ...
  guides(color = guide_colorbar(title = "Water depth (m)")) +
  scale_x_continuous(name = expression(paste(SST[end] - SST[site], " (\u00b0C)"))) +
  scale_y_continuous(name = expression(paste(SST[TEX86] - SST[site], " (\u00b0C)"))) +
...
ggsave("modeled vs tex86 offset - sp6.pdf",tex86_offset_sp6_plot)
```
...
ggsave("travel dist vs tex86 offset - sp6.pdf",tex86_dist_sp6_plot)
```
